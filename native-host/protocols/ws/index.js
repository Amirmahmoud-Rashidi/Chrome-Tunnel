...
//   6. Ping/pong: answered locally; never converted to application data.
//
...
const { stripHopByHop } = require("../../core/headers");
    ...
    relayWsOpen({
      ...
      .then((result) => {
        if (settled || clientSocket.destroyed) {
          if (result.id) relayWsControl({ id: result.id, wsClose: { code: 1000, reason: "Client disconnected during handshake" } }).catch(() => {});
          return;
        }
        if (result.error) {
          logFailedRequest({
        ...
        const clientParser = createFrameParser({ requireMasked: true });
        const ws = {
          remoteClosed: false,
          clientClosed: false,
          closeSent: false,
        ...
        clientSocket.on("data", (chunk) => {
          ...
          let frames;
          try { frames = clientParser.push(chunk); }
          catch (err) {
            ws.clientClosed = true;
            const code = /limit|overflow/.test(err.message) ? 1009 : 1002;
            relayWsControl({ id: result.id, wsClose: { code, reason: "Invalid client WebSocket frame" } }).catch(() => {});
            closeFromRemote(code, "Invalid client WebSocket frame");
            return;
          }
          for (const frame of frames) {
            ...
            if (frame.opcode === OP_PING) {
              ...
              // Browser WebSocket cannot send a protocol ping. Keep it local.
              continue;
            }
            ...
            // The parser reassembles continuations and rejects reserved opcodes.
          }
        });
        ...
        clientSocket.on("close", () => {
          ...
          if (!ws.clientClosed) relayWsControl({ id: result.id, wsClose: { code: 1000, reason: "Client closed" } }).catch(() => {});
          cleanup();
        });
...
function buildTargetUrl(req) {
  try {
    if (req.tunnelAuthority) {
      if (!req.url || !req.url.startsWith("/") || req.url.startsWith("//")) return null;
      return new URL(`wss://${req.tunnelAuthority}${req.url}`).href;
    }
    if (/^(?:wss?|https?):\/\//i.test(req.url || "")) {
    ...
    if (!host || !req.url || !req.url.startsWith("/") || req.url.startsWith("//")) return null;
    return new URL(`ws://${host}${req.url}`).href;
  } catch { return null; }
}

function stripHopByHopHeaders(headers) { return stripHopByHop(headers); }
